...
（`ScreenCore` / `FeatureHome` などが `xcodebuild -list` に並ぶことを確認済み）。
...
| モジュール | 依存 | 置くもの |
| --- | --- | --- |
| `ScreenCore` | なし | 画面の土台。`ViewModel/`（`ViewModel` / `ViewState`）、`Fetch/`（`FetchPhase` / `FetchFailure` / `FetchState`）、`Screen/`（`ScreenViewModel` / `ScreenSource` / `ScreenView` / `ScreenStyle` / `ScreenReloadAction`）に分ける |
...
**公開する View は `ScreenView` だけ。** 描画を担う内部の View は `ScreenView.swift` の
private に閉じてある。`load()` と `reloadID` が internal で外から駆動できない以上、
描画部だけを公開しても使い道が無く、`ScreenView` との取り違えを招くだけだった。

**取得の状態は `FetchState`、画面の UI 状態は `ViewState`。** どちらも「状態」だが別物なので
接頭辞で分けている。**片方だけ修飾しない**——`viewState` / `fetchState` / `fetch()` が
画面ごとに並び、プロパティ名がそのままプロトコル名・型名に対応する。
...
- **画面のオブジェクトは `ViewModel` に準拠させる。** 要求は `viewState`（`ViewState`）・
  `dependency`・`init(dependency:)` の 3 つ。取得がある画面はさらに `ScreenViewModel` に
  準拠し、`fetchState`（`FetchState`）と `fetch()` を足す。**取得の有無で入口を分けない。**
...
- **`fetch()` には取得内容と共通コアの結果からの変換だけを書く。** 要求 ID の照合・
  キャンセル判定・`catch` は `FetchState` が持つので各画面に書かない。表示したい
  失敗メッセージがあるときは `FetchFailure` を throw する。
- **再取得の入口は `\.screenReload` ひとつ。** `ScreenView` が実行経路にだけ実体を流し込み、
  `.snapshot` では何もしない。**失敗表示の再試行ボタンも同じ環境値を使う**ので、
  描画部は再試行のクロージャを引数で受け取らない（実行経路で fetch が 2 回走ることを
  実測で確認済み）。`load()` / `reload()` / `FetchState.run` / `requestReload` /
...
- **`ViewModel` と `ViewState` には無条件で `@Observable` を付ける。**「状態を足すときだけ」に
  しない。**付け忘れても何も言われない。** View が更新されないまま無言で壊れる（body が初回
  1 回しか評価されず画面が固着することを実測で確認済み）。型システムでは強制できない——
  `Observable` は要求を持たないマーカーなので継承させてもマクロ無しで通り、基底クラスに
  付けてもサブクラスの追加プロパティには届かない（いずれも実測で確認済み）。
  マクロで強制する実装も試したが、`@Requires("Observable")` を全型に書かせ、
  プロトコルに意味の分からない要求を 1 つ増やし、swift-syntax 依存と 221 行の
  パッケージを抱える対価に見合わないと判断して外した。**規約で守る。**
  SwiftLint の正規表現では守れなかった——入れ子の型では外側の `@Observable` が近接に
  入るため「どの宣言に付いた属性か」を判定できない（実際に検出できないことを確認済み）。
- **取得状態は `FetchPhase` で表す。** `isLoading` と `error` を別々の変数にすると、
  両方が立った状態が型として表現できてしまう。失敗は `any Error` を保つので、
  認証切れとネットワーク断で表示を変える必要が出ても全画面に波及しない。
- **`.idle` は「まだ取得を始めていない」。`FetchState` の初期値。**
  ...
  **見た目は `.loading` と同じにしてある。** live 経路では `.task` が走る前に `.idle` が
  ...
  そのときに `ScreenStyle` へ `idle` を足して分ける。
- **画面の表示は `ScreenView` に渡す。** 実行する `.live` と状態だけの `.snapshot` を UI 側で
  ...
  画面のライフサイクルから外さないこと。
- **読み込み中と失敗の見た目は `ScreenStyle`。** 各画面が書くのは成功時だけ。
  変えたい範囲に `.screenStyle(_:)` を付ける。`ButtonStyle` と同じ作法で、差し替えは
  modifier を付けた階層に閉じる。**画面ごとに独自の失敗表示を直接書かない。**
- **各画面の `#Preview` は成功時だけ書く。** 読み込み中と失敗は `ScreenStyle` を描いている
  だけで**どの画面でも同じ絵**なので、`ScreenStyle.swift` に 1 度だけ置いてある。
  画面ごとに複製しない。
- **`#Preview` は `.snapshot` を渡す。** `.snapshot` 経路には ViewModel も API も取得タスクも
  ...
  `\.screenReload` は何もしない。動作確認は実行経路で別途行う。
...
テストターゲットは無い。テストを書くときは Xcode でターゲットを追加し、
`make verify` に `test` を足すこと。ターゲットが無い状態で `xcodebuild test` を呼ぶと
